...
def PCA_transcriptomics(trans_data):
	...
	PCA_df = pd.DataFrame(data=pca_components, index=cleanData.index) # , columns=['PCA dimension 1', 'PCA dimension 2'])
...
# define function for Scree plots of PCA data
# plots a full plot and zoomed plot of first n_components
def plot_scree(pca_varRatios, plt_axes, n_components=5):

	'''
	pca_varRatios (pandas DataFrame):		Dataframe containing PCA variance ratios
	plt_axes (matplolib.pyplot Axes obj):	Scree plot axes
	n_components (int): 					Number of components to display in zoomed plot
	'''

	# plot full Scree
	sns.lineplot(data=pca_varRatios, linewidth=1.5, ax=plt_axes[0], legend=False)
	plt_axes[0].set_ylabel('Cumulative Explained Variance', fontsize=14)
	plt_axes[0].set_xlabel('Principal Component', fontsize=14)
	plt_axes[0].set_xticks(np.arange(0,101,10)) 	# format x axis labels to only show every 5th component

	# plot zoomed Scree
	sns.lineplot(data=pca_varRatios, linewidth=1.5, ax=plt_axes[1])
	plt_axes[1].set_ylabel('Cumulative Explained Variance', fontsize=14)
	plt_axes[1].set_xlabel('Principal Component', fontsize=14)

	plt_axes[1].set_xticks(np.arange(0,101,10)) 	# format x axis labels to only show every 5th component
	plt.tight_layout()
	plt.subplots_adjust(wspace=0.3)
	plt.legend(fontsize=4)
	sns.move_legend(plt_axes[1], bbox_to_anchor=(1.0, 1), loc='upper left') 	# move legend

	plt_axes[1].set_xticks(np.arange(0,101,2)) 	# format x axis labels to only show every 5th component
	plt_axes[1].set_ylim(00.60,0.90)
	plt_axes[1].set_xlim(0,n_components)
...
def main():
	...
	# load data
	data_path = "data" 	## !! PLACEHOLDER !! replace this with the path to your data directory
	out_path = "outputs" ## !!PLACEHOLDER!! replace with the path for your data directory
	
	data_f_codes = ['SKCM', 'LIHC', 'PAAD']
	# data_f_codes= ['allDisease']


	tissue_dfs = []
	for f_code in data_f_codes:
		print(f'Running PCA for: TCGA_{f_code}')
		# load data to be summarized by PCA
		gene_exprDF = pd.read_csv(rf'{data_path}\TCGA_{f_code}_rnaseq.csv', index_col=2)
		print(f'gene_exprDF: init:\n{gene_exprDF.iloc[:5,:5]}')
		gene_exprDF.drop(columns=gene_exprDF.columns[[0,1]], inplace=True) 	# remove Gene symbols and NCBI IDs
		print(f'gene_exprDF:\n{gene_exprDF}')

		# Run PCA on transcriptomics data
		base_pcDF = PCA_transcriptomics(trans_data=gene_exprDF)
		cum_varDF = base_pcDF.iloc[1].cumsum()
		var_df = pd.DataFrame(data=[base_pcDF.iloc[1], cum_varDF], index=['Component', 'Cumulative'])
		tissue_dfs.append(var_df)

		base_pcDF.to_csv(f'{data_path}\TCGA_{f_code}_PCA.csv')
	...
	explained_varDF = pd.DataFrame(data=explained_var, index=data_f_codes)
	var_ratioDF = pd.DataFrame(data=var_ratio, index=data_f_codes)

	# plot each subset on our axis
	fig, axes = plt.subplots(1,2, figsize=(12,6))
	plot_scree(var_ratioDF.T, axes, n_components=5)
	plt.savefig(rf'{out_path}\tissue_PCA_Scree_TCGA.png', bbox_inches='tight', dpi=600)
	plt.close()


	#
	# 	FOR GOING FROM FULL CCLE DATASET
	#

	# Compute CCLE baseline (full) dataset PCA
	...
	plt.savefig(rf'{out_path}\tissue_PCA_Scree_CCLE.png', bbox_inches='tight', dpi=600)
	plt.close()

	base_pcDF.to_csv(f'{data_path}\ccleTranscriptomics_PCA.csv')

	# Repeat to get all tissue subset dfs
	# Split across tissues in the dataset, then compute tissue-specific PCAs
	...
	fig, axes = plt.subplots(1,2, figsize=(12,6))
	plot_scree(var_ratioDF.T, axes, n_components=5)
	plt.savefig(rf'{out_path}\tissues_PCA_Scree_CCLE.png', bbox_inches='tight', dpi=600)
	plt.close()
